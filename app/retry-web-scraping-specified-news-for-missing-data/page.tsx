...
import { TextField, Button, Autocomplete, Container, ImageList, ImageListItem, Select, MenuItem, InputLabel } from "@mui/material";
...
export default () => {
    ...
    const handleStartScraping = async () => {
        ...
        try {
            ...
            const response = await fetch(`${NEXT_PUBLIC_BACKEND_URL}/web-scraping/retry-news-urls-where-xxx-is-null-or-the-news-is-native/${mediaName}?filter=${selectedFilter}`, {
                method: "post",
                headers: {
        ...
        <Container>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                    ...
                    <div style={{ display: "flex", gap: 150, alignItems: "center", marginTop: 10 }}>
                        <Title text="Filter by:" />
                        <Select
                            labelId="news-data-select-label"
                            id="news-data-simple-select-for-filtering"
                            value={selectedFilter}
                            style={{ width: 200 }}
                            onChange={(event) => setSelectedFilter(event.target.value as string)}
                        >
